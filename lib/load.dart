...
Future<String> isLoginSuccessful(username,password,gCaptcha,context) async{
  ...
  response = await dio.get(url, queryParameters: {"username": username, "password": password,"captchaResponse":gCaptcha});
    ...
    return Scaffold(
      body: Column(mainAxisAlignment: MainAxisAlignment.center,children: <Widget>[
        ...
        FutureBuilder<String>(
          future: isLoginSuccessful(args.username, args.password, args.captcha,context), // a previously-obtained Future<String> or null
          builder: (BuildContext context, AsyncSnapshot<String> snapshot) {
            switch (snapshot.connectionState) {
              case ConnectionState.waiting:
                return Center(child: Text('Awaiting result...',style: headingStyle,));
              case ConnectionState.done:
                if (snapshot.hasError)
                  return Center(child: Text('Error: ${snapshot.error}'));
                return Center(child: Text('${snapshot.data}',style: headingStyle,));
